...
test("No articles have the same createdAt attribute", async () => {
    ...
    for (const article of getTreeList("./content/articles/en-us")) {
        ...
        const parsedArticle = await markdown.toJSON(
            fs.readFileSync(articlePath, { encoding: "utf-8" })
        );
...
describe("Articles", () => {
    ...
    describe.each(articles)("article %s", (article) => {
        ...
        beforeAll(async () => {
            for (const locale of locales) {
                articleLocales[locale] = await markdown.toJSON(
                    fs.readFileSync(
                        path.join("./content", "articles", locale, article),
                        { encoding: "utf-8" }
                    )
                );
            }
            ...
            for (const locale of locales) {
                ...
                for (const element of elements) {
                    if (
                        ...
                        element.tag === "blog-article-image"
                    ) {
                        ...
                        expect(
                            fs.existsSync(`./static${props["img-path"]}`)
                        ).toBe(true);
        ...
        test("has the same createdAt attribute in all locales", () => {
            for (const locale1 of locales) {
                ...
                for (const locale2 of locales) {
                    expect(articleLocales[locale1].createdAt).toEqual(
                        articleLocales[locale2].createdAt
                    );
                }
            }
        });
            ...
            for (const locale1 of locales) {
                ...
                expect(
                    fs.existsSync(
                        path.join(
                            "./static",
                            articleLocales[locale1].previewImg
                        )
                    )
                ).toBe(true);
                for (const locale2 of locales) {
                    expect(articleLocales[locale1].previewImg).toEqual(
                        articleLocales[locale2].previewImg
                    );
                }
            }
        });
